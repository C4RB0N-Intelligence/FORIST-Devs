...
/**
 * FormInput — label sits above the field. Subtle background, clean border,
 * accent-colored focus border (no glow-only focus state). Password fields
 * get a built-in show/hide toggle.
 */
const FormInput = forwardRef<HTMLInputElement, FormInputProps>(function FormInput(
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label
        htmlFor={inputId}
        className="text-sm font-medium text-[#1C1A17] dark:text-[#F2EFE9]"
      >
        {label}
      </label>
      ...
      <div className="relative">
        <input
          ...
          className={[
            "h-11 w-full rounded-lg border px-3.5 text-[15px] leading-none",
      ...
      {error && (
        <p
          id={`${inputId}-error`}
          className="text-sm text-[#8C2F2F] dark:text-[#E17272]"
        >
          {error}
        </p>
      )}
